...
class Carta:
    ...
    def dibujar(self, ventana):
        if self.animacion_volteo is not None:
            ...
            # Suavizado tipo ease: ancho va 1 -> 0 -> 1 (seno)
    ...
    def clic_en_carta(self, pos):
        return (self.rect.collidepoint(pos) and 
                ...
                not self.emparejada and
                self.animacion_volteo is None)
...
def dibujar_barra_tiempo(ventana, tiempo_restante, tiempo_limite, fuente):
    ...
    pygame.draw.rect(ventana, constantes.BARRA_TIEMPO_FONDO, rect_fondo)
    pygame.draw.rect(ventana, constantes.BLANCO, rect_fondo, 2)
    if tiempo_limite and tiempo_limite > 0 and tiempo_restante is not None:
        ...
        ancho_lleno = barra_ancho * p  # float para transición suave
        if ancho_lleno > 0:
            pygame.draw.rect(ventana, color_barra, (x, y, int(ancho_lleno), barra_alto))
...
def ejecutar_juego(ventana, reloj, dificultad, tiempo_segundos=None):
    # Tiempo: lo que eligió el usuario (None = sin tiempo, o 60, 120, 180, custom)
    ...
    esperando_voltear_atras = False  # esperando que terminen de ocultarse las 2 cartas
    ...
    while jugando:
        ...
        for evento in pygame.event.get(): 
            ...
            if (evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1 
                ...
                # Clic en botones (no se usa efecto carta, sí botón)
        ...
        texto_prog = fuente.render(f"{pares_encontrados}/{num_pares} pares", True, constantes.MORADO_P)
        ventana.blit(texto_prog, (10, 40))
        
        # Barra de tiempo en la parte superior (centro)
        dibujar_barra_tiempo(ventana, tiempo_restante if tiempo_restante is not None else 0.0, tiempo_limite, fuente)

        # Victoria (solo si completó todos los pares y no se acabó el tiempo)
        if pares_encontrados == num_pares and not tiempo_agotado:
            ...
            texto_sub = fuente_sub.render("R = Reiniciar nivel  |  ESC = Menú  |  Enter = Siguiente nivel", True, constantes.BLANCO)
                ...
                for evento in pygame.event.get():
                    ...
                    if evento.type == pygame.KEYDOWN:
                        ...
                        if evento.key == pygame.K_RETURN:
                            return "menu"  # Siguiente nivel = volver al menú para elegir otro
        ...
        # Derrota por tiempo: solo Reiniciar nivel y Menú principal
        elif tiempo_agotado:
            ...
            texto_sub_d = fuente_sub_d.render("Se acabo el tiempo, vuelve a intentarlo", True, constantes.BLANCO)
